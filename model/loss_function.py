...
class MultiBoxLoss(nn.Module):
    ...
    def __init__(self, jaccard_thresh=0.5, neg_pos=3, device='cpu'):
        ...
        self.jaccard_thresh = jaccard_thresh #0.5関数のjaccard係数の閾値
        self.negpos_ratio = neg_pos #3:1 Hard Negative Miningの負と正の比率
        self.device = device #cPUとGPUのいずれかで計算するか
    ...
    def forward(self, predictions, targets):
        ...
        for idx in range(num_batch):#ミニバッチでループ
            ...
            #conf_t_label : 各BBoxに一番近いBBoxのラベルが上書きされる
        ...
        # ----------
        #クラス予測の損失：loss_cを計算
        #交差エントロピー誤差関数で損失を計算する。
        #ただし、背景クラスが正解であるDBoxが圧倒的に多いので、
        #Hard Negative Miningを実施し、物体発見DBoxと背景クラスDBoxの比が1:3になるようにする
        #そこで背景クラスDBoxと予想したもののうち、損失が小さいものはクラスの損失から除く
        # ----------
        batch_conf = conf_data.view(-1, num_classes)

        #クラス予測の損失関数を計算(reduction='none'にして、和をとらず、次元を潰さない)
        loss_c = F.cross_entropy(
                batch_conf, conf_t_label.view(-1), reduction='none')

        # ----------
        #これからNegative DBoxのうち、
        #Hard Negative Miningで抽出するものを求めるマスクを作成する
        # ----------
        ...
        #los_c[pos_mask] = 0 #物体を発見したDBoxは損失0とする

        #Hard Negative Miningを実施する
        #各DBoxの損失の大きさloss_cの順位であるidx_rankを求める.torch型.
        _, loss_idx = loss_c.sort(1, descending=True)
        _, idx_rank = loss_idx.sort(1)

        #(注釈)
        #実装コードが特殊で直感的ではないです。
        #上記２行は、要は各DBoxに対して、損失の大きさが何番めなのかの情報を
        #変数idx_rankとして高速に取得したいというコードです。
        #
        #DBOXの損失値の大きい方から降順に並べ、DBoxの降順のindexをloss_idxに格納
        #損失の大きさloss_cの順位であるid_rankを求める
        #ここで、
        #降順になった配列indexであるloss_idxを、0から8732まで昇順に並べなおすには、
        #何番めのloss_idxのインデックスを取ってきたら良いかを示すのがldx_rankである。
        #例えば、
        #idx_rankの要素0番目 = idx_rank[0]を求めるにはloss_idxの値が0の要素、
        #つまりはloss_idx[?]=0の、?は何番目かを求めることになる。ここで,? = idx_rank[0]である。
        ...
        #背景のBoxの数num_negを決める。HardNegative Miningにより、
        #物体発見のDBoxの数num_posの3倍(self.negpos_ratio倍)とする。
        #ただし、万が一、DBoxを超える場合は、DBoxの数を上限とする。
        num_neg = torch.clamp(num_pos*self.negpos_ratio, max=num_dbox)

        #idx_rankは各DBoxの損失の大きさが上から何番目なのかが入っている
        #背景のDBoxの数num_negよりも、順位が低い(すなわち損失が大きい)DBoxをとるマスク作成
        #torch.Size([num_batch, 8732])
        neg_mask = idx_rank < (num_neg).expand_as(idx_rank)

        # ----------
        #(終了)ここからNegative DBoxのうち、Hard Negative Miningで抽出するものを
        #求めるマスクを作成します。
        # ----------
        ...
        #形はtorch.Size([num_pos+num_neg, 21])
        conf_hnm = conf_data[(pos_idx_mask+neg_idx_mask).gt(0)].view(-1, num_classes)

        #(注釈)gtはgreater than (>)の略称。これでmaskが1のindexを取り出す。
        #pos_idx_mask+neg_idx_maskは足し算だが、indexへのmaskをまとめているだけである.
        #つまり、posであろうがnegであろうが、マスクが1つのものを足し算で1つのリストにし、それをgtで取得。        
